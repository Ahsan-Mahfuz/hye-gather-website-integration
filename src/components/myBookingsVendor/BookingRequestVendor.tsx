...
interface BookingRequestProps {
  user: string // User ID from props
  vendorId?: string
  categoryId?: string
}
...
const BookingRequestVendor: React.FC<BookingRequestProps> = ({ user }) => {
  ...
  const businessData = getBusinessServiceData?.data?.[0]
  const businessServices = businessData?.business_services || []
  const businessCategory = businessData?.business_category
  ...
  // Update formData when business data is loaded
  useEffect(() => {
    if (businessData) {
      setFormData((prev) => ({
        ...prev,
  ...
  const validateStep = (): boolean => {
    const validations: Record<number, boolean> = {
      ...
      4: !!formData.duration,
      5: true,
      6: formData.services.length > 0 && !!price,
    }
  ...
  return (
    <>
      <div
        ...
        className="relative  px-8 py-4 mt-4 text-white font-bold text-lg rounded-full 
